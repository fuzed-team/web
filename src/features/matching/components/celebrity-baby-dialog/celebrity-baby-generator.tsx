...
import {
	useCelebrityBaby,
	useGenerateCelebrityBaby,
} from "../../api/generate-celebrity-baby";

interface CelebrityBabyGeneratorProps {
...
export const CelebrityBabyGenerator = ({
	celebrityMatchId,
	userPhoto,
	celebrityPhoto,
	userName,
	...
	matchPercentage,
	onBack,
}: CelebrityBabyGeneratorProps) => {
	const [babyImage, setBabyImage] = useState<string>("");

	const generateBabyMutation = useGenerateCelebrityBaby({
		mutationConfig: {
			onSuccess: (data) => {
				if (!data?.image_url) {
				...
				toast.success("Baby generated with celebrity! 👶✨");
			},
		},
	});

	// Fetch existing baby
	const { data: existingBaby, isLoading: loadingExisting } = useCelebrityBaby({
		celebrityMatchId,
	});

	useEffect(() => {
		if (existingBaby?.baby?.image_url) {
			setBabyImage(existingBaby.baby.image_url);
		}
	}, [existingBaby]);
	...
	const handleGenerate = async () => {
		...
		if (!celebrityMatchId) {
			toast.error("Celebrity match is required to generate baby! 📸");
			return;
		}
		setBabyImage("");
		generateBabyMutation.mutate(celebrityMatchId);
	};

	const shareBaby = async () => {
		if (!babyImage) return;
		const shareData = {
			title: `My Baby with ${celebrityName}! 👶`,
			text: `Check out what my baby with ${celebrityName || "a celebrity"} would look like! 💕 #Fuzed`,
		...
		try {
			if (
				...
				await navigator.clipboard.writeText(
					`Check out my baby with ${celebrityName || "a celebrity"}! ${window.location.href}`,
				);
	...
	const saveBaby = async () => {
		...
		try {
			...
			link.download = `fuzed-baby-${celebrityName || "celebrity"}.jpg`;
			...
			<div className="relative z-10 w-full flex flex-col">
				...
				<div className="px-6 sm:px-8 pt-8 pb-10 sm:pb-6 text-center">
					<div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-[11px] font-semibold tracking-wider uppercase mb-3">
						<Sparkles className="w-3 h-3" /> Celebrity Baby
					</div>
					<h2 className="text-2xl font-bold tracking-tight text-foreground mb-1">
						Celebrity Future Generator
					</h2>
					<p className="text-muted-foreground text-sm">
						See what your baby with {celebrityName || "this celebrity"} would
						look like!
					</p>
				</div>
