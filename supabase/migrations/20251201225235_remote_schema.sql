...
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();
  ...
  for insert
  to authenticated
with check ((bucket_id = 'user-images'::text));
...
using ((bucket_id = 'user-images'::text));
  ...
  for delete
  to authenticated
using (((bucket_id = 'user-images'::text) AND ((storage.foldername(name))[1] = (auth.jwt() ->> 'email'::text))));
  ...
  to authenticated
using (((bucket_id = 'user-images'::text) AND ((storage.foldername(name))[1] = (auth.jwt() ->> 'email'::text))))
with check (((bucket_id = 'user-images'::text) AND ((storage.foldername(name))[1] = (auth.jwt() ->> 'email'::text))));
